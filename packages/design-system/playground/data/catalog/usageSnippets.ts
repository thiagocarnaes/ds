...
export const usageSnippets: Record<string, string> = {
  ...
  Radio: usage('Radio', '  <Radio v-model="plan" value="pro" label="Pro" :disabled="false" />'),
  ...
  Label: usage('Label', '  <Label for="email" required>Email</Label>'),
...
<!--
<Toast variant="success" dismissible>
  Changes saved.
</Toast>
-->
...
<template>
  ...
  <ToastHost />
  <Button appearance="primary" @click="toast.info('Hello', { position: 'top-right' })">
    Show toast
  </Button>
</template>
...
toast.info('Hello', { position: 'top-right', dismissible: true })`,
  ...
  SidebarMenuGroup: usageMany(
    ...
    `  <SidebarMenuGroup id="forms" label="Forms" default-open>
  ...
  Lozenge: usage(
    'Lozenge',
    `  <Lozenge appearance="success" :bold="false">
    Active
  </Lozenge>`,
  ),
  Table: usageMany(
